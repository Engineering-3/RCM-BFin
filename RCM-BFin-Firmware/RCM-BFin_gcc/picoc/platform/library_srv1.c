...
void Canalog(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)  // return reading from HMC6352 I2C compass
{
    ...
    i2cwritex(device_id, (unsigned char *)i2c_data, 3, SCCB_OFF);
...
/* nothing here because we don't add any functions until rcm-bfin.h is #included */
struct LibraryFunction PlatformLibrary[] =
{
    { NULL,         NULL }
};

/* list of all library functions included with rcm-bfin.h */
struct LibraryFunction RCMBFINFunctions[] =
{
    { Csignal,      "int signal();" },
    { Cinput,       "int input();" },
...
{
    IncludeRegister("rcm-bfin.h", &RCMBFINSetupFunc, &RCMBFINFunctions[0], NULL);
}
