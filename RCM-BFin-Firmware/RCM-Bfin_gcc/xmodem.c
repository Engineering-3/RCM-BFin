...
// Wait for up to timeout milliseconds to get a character.
// If character received in that time, then return character
// If timeout, then return -1
int32_t _inbyte(unsigned int timeout) 
{
    unsigned char ch;
    XMODEMTimeoutMS = timeout;
    while (XMODEMTimeoutMS)
    {
        if (uart0GetChar(&ch))
        {
            return ch;
...
unsigned short crc16_ccitt(const void *buf, int len)
{
    ...
    for( counter = 0; counter < len; counter++)
	{
		...
		{
            char tempbuf[32];
			sprintf(tempbuf, "CRC: byte %d is %02X\n\r",counter, *(char *)buf);
			uart1SendString((unsigned char *)tempbuf);
			tmp1 = crc >> 8 ^ *(char *)buf;
			tmp2 = tmp1 & 0x00FF;
			tmp3 = crc16tab[tmp2];
			tmp4 = crc << 8;
			tmp5 = tmp4 ^ tmp3;
			sprintf(tempbuf, "CRC: tmp1 = %04X\n\r", tmp1);
			uart1SendString((unsigned char *)tempbuf);
			sprintf(tempbuf, "CRC: tmp2 = %04X\n\r", tmp2);
			uart1SendString((unsigned char *)tempbuf);
			sprintf(tempbuf, "CRC: tmp3 = %04X\n\r", tmp3);
			uart1SendString((unsigned char *)tempbuf);
			sprintf(tempbuf, "CRC: tmp4 = %04X\n\r", tmp4);
			uart1SendString((unsigned char *)tempbuf);
			sprintf(tempbuf, "CRC: tmp5 = %04X\n\r", tmp5);
			uart1SendString((unsigned char *)tempbuf);
		}
...
// Performs CRC check. 
// crc is 0 to use simple 8-bit sum, otherwise use crc16_citt for CRC
// *buf is a pointer to the data
// sz is the size of the data buffer, and also an index into *buf of where the received CRC is
// Returns 1 if CRC matches, 0 if it doesn't.
...
// Keep eating input bytes until there is a delay of 50ms with no
// new bytes coming in.
static void flushinput(void)
{
    while (_inbyte(DLY_50MS) >= 0)
    {
        ;
    }
}
        ...
        for( retry = 0; retry < 30; ++retry)      // approx 30 seconds allowed to make connection
        {
            ...
            if ((c != -1))
            {
                ...
                case EOT:
                    ...
                    PacketEnd(true);
#if defined(XDEBUG)
printf("BadInv = %d BadNum = %d BadCRC = %d Retry = %d\r\n", BadInv, BadNum, BadCRC, Retransmits);
#endif
...
printf("BadInv = %d BadNum = %d BadCRC = %d Retry = %d\r\n", BadInv, BadNum, BadCRC, Retransmits);
    ...
    start_recv:
        ...
        p = xbuff;
        *p++ = c;
        // Wait for bufsz bytes from PC, or timeout
