...
int spi_write (unsigned int offset, unsigned char *buffer, unsigned char *temp, int len)
{
    ...
    if(result == INVALID_SECTOR) {
        printf("spi_write(): start block is Invalid sector! ");
        return 0;
    }
    /* Get the end block number */
    result = GetSectorNumber(offset + len -1, &end_block);
    if(result == INVALID_SECTOR) {
        printf("spi_write(): end block is Invalid sector! ");
        return 0;
    }
    ...
    for(num = start_block;num <= end_block;num ++){
		...
		#ifdef FLASH_DEBUG
			printf("Wrote block %d, %0Xu %0Xu %p\r\n", num, num*SECTOR_SIZE, SECTOR_SIZE, (int *)temp);
		#endif
		
        if(result != NO_ERR)
		{
			printf("spi_write(): WriteData() returned error %d\r\n", result);
            return 0;
		}
        //printf(".");
    }
...
ERROR_CODE WriteData( unsigned long ulStart, long lCount, int *pnData )
{
    ...
    {
		#ifdef FLASH_DEBUG
			printf("About to write start %08lX len %08lX data %08X\r\n", ulWStart, lWCount, *pnData);
		#endif
		
		ErrorCode = WriteFlash(ulWStart, lWCount, pnData, pnWriteCount);

		#ifdef FLASH_DEBUG
			printf("Done writing %08lX bytes, error code = %d\r\n", *pnWriteCount, ErrorCode);
		#endif
