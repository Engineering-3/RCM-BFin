/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *  gps.c - navigation functions for the RCM-Bfin / SVS Blackfin controllers
 *
...
unsigned int GPSisqrt(unsigned int val) {
...
int gps_head(int lat1, int lon1, int lat2, int lon2)
{
    ...
    if (ll < 0) 
        ll = -lat1;
    x = ((lon2 - lon1) * GPScos(ll / 1000000)) / 1000;
    if (x < 0) { sx = 1; x = -x; }
...
int gps_dist(int lat1, int lon1, int lat2, int lon2)
{
    ...
    if ((x > 10000) || (y > 10000)) {
        x = x / 10000;
        y = y / 10000;
        return GPSisqrt(x*x + y*y) * 1113;   
    } else {
        return (GPSisqrt(x*x + y*y) * 1113) / 10000;   
    }
}
