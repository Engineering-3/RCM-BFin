/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *  httpd.c - HTTP GET and POST functions for the RCM-Bfin / SVS 
 *
...
#define USER_FLASH_SECTORS      2

#define BOOT_LOADER_SECTORS     4
#define BOOT_LOADER_MIN_SIZE    (16 * 1024)
#define BOOT_LOADER_MAX_SIZE    (BOOT_LOADER_SECTORS * ONE_SECTOR)
...
void    httpd_request (char firstChar)
{
    ...
    if (reqBuf[0] == 'G') {
        ...
        while (readRTC() - t0 < 1000  &&  ret < REQBUF_SIZE) {
            if (uart0GetChar((unsigned char *) &ch))
            {
...
//                    break; 
                ...
                case 'i': 
                    switch (params[1]) {
                        ...
                        case 'R':  // double byte (short) I2C read
                            ...
                            new_content = 1;
                            break;
                        case 'w':  // single byte (register and data) I2C write
                            i2c_device = (unsigned char)atoi_b16(&params[2]);
                            i2c_data[0] = (unsigned char)atoi_b16(&params[4]);
                            i2c_data[1] = (unsigned char)atoi_b16(&params[6]);
                            i2cwrite(i2c_device, (unsigned char *)i2c_data, 1, SCCB_OFF);
                            break;
                        case 'W':  // double byte (short) (register and 2 data bytes) I2C write
                            i2c_device = (unsigned char)atoi_b16(&params[2]);
                            i2c_data[0] = (unsigned char)atoi_b16(&params[4]);
                            i2c_data[1] = (unsigned char)atoi_b16(&params[6]);
                            i2c_data[2] = (unsigned char)atoi_b16(&params[8]);
                            i2cwritex(i2c_device, (unsigned char *)i2c_data, 3, SCCB_OFF);
                            break;
                ...
                case 'l':  // l1 = lasers on, l0 = lasers off
                    if (params[1] == '1')
                        ...
                        *pPORTHIO &= 0xFD7F;
                    break;
                case 'm':  // RCM-Bfin Robot motor drive (PWM)
...
// Commented this out on 04/25/2012 because admin page allows updating firmware image with no checksum
// or error checking of any kind, thus allowing corrupted images to brick the RCM-Bfin.
#if 0
    ...
    //else if (strcmp (method, "POST") == 0) {
        ...
        if (strncmp(path, adminPath, countof(adminPath) - 1) == 0) {
            ...
            if (reqContentLength  &&  strcmp (type, "multipart/form-data") == 0  &&  
                            ...
                            *secBody = 0;   // terminate section headers
                            ...
                            if (getHdrString (secStart, "Content-Disposition: ", contDisp, countof (contDisp)) &&
                                ...
                                // sectorStart
                                    ...
                                    if (sectorStart < USER_FLASH_SECTOR_BEGIN  ||  sectorStart > USER_FLASH_SECTOR_END-1)
                                        sectorStart = -1;
                                }
                ...
                if (reqBodyCount != reqContentLength)
                ...
                else if (uploadDest == toSectors  &&  fileBodyLength > USER_FLASH_SECTORS * ONE_SECTOR)
                    sprintf (resultMsg, "File size (%d) too big for %d sectors", fileBodyLength, USER_FLASH_SECTORS);
                    ...
                    {
                        flashAddress = sectorStart * ONE_SECTOR;
                        bytesToWrite = USER_FLASH_SECTORS * ONE_SECTOR;
                    }
                    else
                    {
                        flashAddress = BOOT_FLASH;
                        bytesToWrite = BOOT_LOADER_SECTORS * ONE_SECTOR;
                    }
